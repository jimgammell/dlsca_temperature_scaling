...
import datasets, datasets.common
...
class ClassifierTrainer:
    def __init__(
        ...
        for var_name, var in locals():
            setattr(self, var_name, var)
        
        # Ensure arguments are valid
        assert self.dataset_name in dataset.AVAILABLE_DATASETS
        assert isinstance(dataset_kwargs, dict)
        assert self.model_name in models.AVAILABLE_MODELS
        ...
        assert isinstance(self.training_metric, dict) \
    ...
    def reset(self, epochs=None):
        ...
        self.train_dataset, self.val_dataset = torch.utils.data.random_split(
            self.train_dataset, (len(train_dataset)-val_split_size, val_split_size)
        )
        ...
        self.train_sample_transforms = Compose([getattr(datasets.transforms, tf) for tf in self.train_sample_transforms])
        self.train_target_transforms = Compose([getattr(datasets.transforms, tf) for tf in self.train_target_transforms])
        self.train_batch_transforms = Compose([getattr(datasets.transforms, tf) for tf in self.train_batch_transforms])
        self.eval_sample_transforms = Compose([getattr(datasets.transforms, tf) for tf in self.eval_sample_transforms])
        self.eval_target_transforms = Compose([getattr(datasets.transforms, tf) for tf in self.eval_target_transforms])
        self.eval_batch_transforms = Compose([getattr(datasets.transforms, tf) for tf in self.eval_batch_transforms])
        self.train_dataset.dataset.transform = self.train_sample_transforms
        self.train_dataset.dataset.target_transform = self.train_target_transforms
        self.val_dataset.dataset.transform = self.eval_sample_transforms
        self.val_dataset.dataset.target_transform = self.eval_target_transforms
        self.test_dataset.transform = self.eval_sample_transforms
        self.test_dataset.target_transform = self.eval_target_transforms
        self.train_dataloader = datasets.get_dataloader(self.train_dataset, batch_size=self.batch_size, shuffle=True)
        self.val_dataloader = datasets.get_dataloader(self.val_dataset, batch_size=self.batch_size, shuffle=False)
        self.test_dataloader = datasets.get_dataloader(self.test_dataset, batch_size=self.batch_size, shuffle=False)
        self.model = models.construct_model(self.model_name, **self.model_kwargs)
        ...
        if self.use_sam:
            ...
            self.optimizer = getattr(optim, self.optimizer_class)(model.parameters(), **self.optimizer_kwargs)
    ...
    def train_step(self, batch):
        ...
        start.record()
        rv = train.common.ResultsDict() # Store training metrics recorded for this batch
        self.model.train() # Set model to training mode (e.g. enable dropout, batchnorm uses single-minibatch stats)
        traces, labels = train.common.unpack_batch(batch, self.device) # Move batch to appropriate device (probably a GPU)
        if self.train_batch_transforms is not None: # Apply data transforms that operate on the full minibatch, e.g. mixup, cutmix
            traces, labels = self.train_batch_transforms(traces, labels)
        ...
        def closure(
            ...
            if update_rv:
                ...
                for metric_name, metric_fn in self.train_metrics.items(): # Compute additional metrics
                    rv[metric_name] = metric_fn(logits, labels)
                norms = train.common.get_norms(self.model) # Compute model parameter weight/gradient norms for debugging
                rv.update(norms)
        ...
        if self.use_sam: # Do the optimizer step, which will entail more forwards/backwards passes for e.g. SAM
            ...
            self.optimizer.step()
        if self.scheduler is not None: # Update the learning rate if we are using a scheduler
    ...
    def eval_step(self, batch):
        ...
        traces, labels = train.common.unpack_batch(batch, self.device) # Move batch to appropriate device
        if self.eval_batch_transforms is not None: # Apply data transforms that operate on the full minibatch
            traces, labels = self.eval_batch_transforms(traces, labels)
        logits = self.model(traces) # Do the forward pass
    ...
    def update_bn_stats(self):
        ...
        train_dataloader_transforms = self.train_dataloader.dataset.dataset.transforms
        self.train_dataloader.dataset.dataset.transforms = self.val_dataloader.dataset.dataset.transforms
        ...
        for batch in self.train_dataloader: # Do a forward pass for each batch, which will automatically update the batchnorm statistics.
            traces, _ = train.common.unpack_batch(batch, self.device)
            _ = self.model(traces)
        self.train_dataloader.dataset.dataset.transforms = train_dataloader_transforms
